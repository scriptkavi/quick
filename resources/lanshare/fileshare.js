...
function recursiveReaddir(targetPath, basePath, name = null, orderByTime = false) {
  return readdirPromise(targetPath)
    .then((contentNames) => {
      return Promise.all(
        contentNames.map((fileOrDirName) => {
    ...
    .then((contentObjects) => {
      if (contentObjects.length == 0 && targetPath != basePath) {
        // Ignoring empty (sub)folders
        return null
      } else {
        let timestamp = null
...
const logDebug =
  process.env.NODE_ENV === 'debug'
    ? (msg, ...args) => {
        console.log(msg, ...args.map((arg) => util.inspect(arg, false, 10, true)))
      }
    : (msg, ...args) => {}
...
class LiveCache {
  constructor(filesFolderPath, orderByTime = true) {
    ...
    function addFolderToCache(pathStr, stats) {
      if (pathStr == '') {
        rootContent['timestamp'] = Math.max(
          ...[stats.ctimeMs, stats.mtimeMs].filter((x) => x != null)
        )
      } else {
  ...
  prepContentOutput() {
    return this.contentPrepPromise.then(() => {
      ...
      if (this.contentOutputMD5 == null) {
        ...
        this.contentOutputMD5 = crypto
          .createHash('md5')
          .update(JSON.stringify(this._contentOutputJSON))
          .digest('hex')
      }
...
function customizeQRCode(bitmap) {
  ...
  return new Promise((resolve, reject) => {
    fs.exists(imagePath, function (exists) {
      if (exists) {
        ...
        let qr_svg = qr_image.image(`http://${address}:${port}/`, {
          type: 'png',
          customize: customizeQRCode
        })
        qr_svg
...
module.exports = function (conf) {
  ...
  app.get('/delete/:filename', function (req, res) {
    ...
    if (allowDeletion) {
      ...
      if (fileFullPath != filesFolderFullPath && fileFullPath.startsWith(filesFolderFullPath)) {
        try {
  ...
  app.get('/info', function (req, res) {
    ...
    Promise.all([addressesPromise, rootContentPromise]).then(
      ([addresses, [rootContent, rootContentMD5]]) => {
  ...
  app.use(function (req, res, next) {
    if (errorCallback) {
      // NOTE(baris): Preserved for backwards compatibility.
